...
def main(fileName1=None):

    isFileName1NotEmpty = fileName1 != None
    # logged data file name for attack injection campaign
    campaign_log_file = 'ComFASE Attack Injection Campaign Log _2024-09-11 15.46.49.csv'
    # Reads fcd.xml file for the Golden Run experiment
    file_GoldenRun = sorted(glob.glob("GoldenRun_sinusoidal/*fcd-output.xml*"))
    ...
    # Reads all results files with extension of .sca, vci and vec and lists them
    file_results = sorted(glob.glob("results/*.sca"))
    file_results.extend(sorted(glob.glob("results/*.vci")))
    file_results.extend(sorted(glob.glob("results/*.vec")))

    # Current time extraction
    now = datetime.now()
    current_time = now.strftime("%Y-%m-%d %H.%M.%S")
    fileName2 = "Parsed Accel-Deceleration_All Vehicles_{} ParsedData.csv".format(current_time)
    ...
    print("COLLISION files= ", file_list_collision)
    ...
    #        if timee>=11.00:
        ...
        filePathAbs = str(Path(file).absolute())
        filePathNew = str(filePathAbs).split(file)[0] + "ComFASE_data/" + fileName1[0:-4]
        pathObj = Path(filePathNew)
        if not pathObj.exists():
            Path.mkdir(pathObj)
            print(filePathNew + " created")
        Path(filePathAbs).rename(filePathNew + "/" + str(file).split("sumocfg/")[1])
    ...
    for file in file_list_collision:
        ...
        if isFileName1NotEmpty:
            filePathAbs = str(Path(file).absolute())
            filePathNew = str(filePathAbs).split(file)[0] + "ComFASE_data/" + fileName1[0:-4]
            Path(filePathAbs).rename(filePathNew + "/" + str(file).split("sumocfg/")[1])
            print(filePathAbs + " moved to " + filePathNew + "/" + str(file).split("sumocfg/")[1])

    if isFileName1NotEmpty:
        for file in file_results:
            filePathAbs = str(Path(file).absolute())
            filePathNew = str(filePathAbs).split(file)[0] + "ComFASE_data/" + fileName1[0:-4]
            Path(filePathAbs).rename(filePathNew + "/" + str(file).split("results/")[1])
    #==================================================================================#
    ...
    for row in readCSV:
        ...
        #startTime.append(float(row[3]))
        #endTime.append(float(row[1]))
    ...
    df_7.to_csv(fileName2)

    if isFileName1NotEmpty:
        filePathAbs = str(Path(fileName1).absolute())
        filePathNew = str(filePathAbs).split(fileName1)[0] + "ComFASE_data/" + fileName1[0:-4]
        Path(filePathAbs).rename(filePathNew + "/" + str(fileName1))
